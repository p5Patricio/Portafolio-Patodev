...
// ----- Brand colors -----
//
// Official brand hex per tech, used to tint each icon in the Stack section.
// Omitted (falls back to `--color-paper`) when the official color is
// black/near-black or otherwise too dark to read at >=3:1 contrast against
// the `ink` (#0E1014) background — e.g. Next.js, Vercel, GitHub, Notion,
// OpenAI, Ollama, Render, Pandas, NumPy — or when no reliable official brand
// color exists (custom/AI-tool icons, LaTeX, etc).
// eslint-disable-next-line react-refresh/only-export-components
export const TECH_COLORS: Partial<Record<TechId, string>> = {
  react: '#61DAFB',
  vue: '#4FC08D',
  ts: '#3178C6',
  js: '#F7DF1E',
...
type Props = {
  id: TechId
  className?: string
  /** Override the rendered accessible label (defaults to TECH_LABELS[id]). */
  label?: string
}

/**
 * Brand-colored tech icon. Flat, no glow/shadow — colored via inline style
 * (falls back to `--color-paper` when the official brand color is too dark
 * to read on the `ink` background, or unknown).
 *
 * Usage:
 *   <TechIcon id="react" className="h-9 w-9" />
 */
function TechIcon({ id, className = 'h-8 w-8', label }: Props) {
  const Icon = ICONS[id] ?? FallbackIcon
  const accessibleLabel = label ?? TECH_LABELS[id]
  const color = TECH_COLORS[id] ?? 'var(--color-paper)'
